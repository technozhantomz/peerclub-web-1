<script>
	...
	let store_login_user_uuid = get(login_user_uuid);
	//console.log(store_login_user_uuid);
	...
	//Getting User Data
	async function userGetProfileData() {
		const res = await fetch(store_server_url + 'getusers', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
			body: JSON.stringify({
				user_uuid: store_login_user_uuid
			})
		});
	...
	async function userChangePassword() {
		const res = await fetch(store_server_url + 'opsusers', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
...
<svelte:head>
	<title>Profile Settings | Homepesa Mega Platform</title>
</svelte:head>
	...
	<Container fluid>
		<div class="position-relative mx-n4 mt-n4">
			<div class="profile-wid-bg profile-setting-img">
				...
				<!--
				<Avatar bgColor="#7084c7" name={store_glb_username} />
				-->
				<!--
				...
				<Card class="mt-n5">
					<CardBody class="p-4">
						<div class="text-center">
							...
							<h5 class="fs-17 mb-1">{store_glb_username}</h5>
							...
							<TabPane tabId={2} class={activeTab == 2 ? 'active' : ''}>
								<Form>
									<Row class="g-2">
										...
										<Col lg={12}>
											<div class="mb-3">
												<Link href={null} class="link-primary text-decoration-underline"
							...
							<TabPane tabId={4} class={activeTab == 4 ? 'active' : ''}>
								...
								<div class="mb-3">
									...
									<ul class="list-unstyled mb-0">
										<li class="d-flex">
											<div class="flex-grow-1">
												<Label for="directMessage" class="form-check-label fs-15"
