<script>
	import Link from 'svelte-link';
	import { Card, Col, Container, Row, Label } from 'sveltestrap';

	import AuthSlider from '../AuthSlider.svelte';
</script>

<svelte:head>
	<title>Sign Up | Peer Club Dashboard</title>
</svelte:head>
<div
	class="auth-page-wrapper auth-bg-cover py-5 d-flex justify-content-center align-items-center min-vh-100"
>
	<div class="bg-overlay" />
	<div class="auth-page-content overflow-hidden pt-lg-5">
		<Container>
			<Row>
				<Col lg={12}>
					<Card class="overflow-hidden m-0">
						<Row class="justify-content-center g-0">
							<AuthSlider />

							<div class="col-lg-6">
								<div class="p-lg-5 p-4">
									<div>
										<h5 class="text-primary">Register Account</h5>
										<p class="text-muted">Get your Free Peer Club account now.</p>
									</div>

									<div class="mt-4">
										<form class="needs-validation" noValidate action="index">
											<div class="mb-3">
												<Label for="useremail" class="form-label"
													>Email <span class="text-danger">*</span></Label
												>
												<input
													type="email"
													class="form-control"
													id="useremail"
													placeholder="Enter email address"
													required
												/>
												<div class="invalid-feedback">Please enter email</div>
											</div>
											<div class="mb-3">
												<Label for="username" class="form-label"
													>Username <span class="text-danger">*</span></Label
												>
												<input
													type="text"
													class="form-control"
													id="username"
													placeholder="Enter username"
													required
												/>
												<div class="invalid-feedback">Please enter username</div>
											</div>

											<div class="mb-2">
												<Label for="userpassword" class="form-label"
													>Password <span class="text-danger">*</span></Label
												>
												<input
													type="password"
													class="form-control"
													id="userpassword"
													placeholder="Enter password"
													required
												/>
												<div class="invalid-feedback">Please enter password</div>
											</div>

											<div class="mb-4">
												<p class="mb-0 fs-12 text-muted fst-italic">
													By registering you agree to the Peer Club <Link
														href=""
														class="text-primary text-decoration-underline fst-normal fw-medium"
														>Terms of Use</Link
													>
												</p>
											</div>

											<div class="mt-4">
												<button class="btn btn-success w-100" type="submit">Sign Up</button>
											</div>

											<div class="mt-4 text-center">
												<div class="signin-other-title">
													<h5 class="fs-13 mb-4 title text-muted">Create account with</h5>
												</div>

												<div>
													<button
														type="button"
														class="btn btn-primary btn-icon waves-effect waves-light me-1"
														><i class="ri-facebook-fill fs-16" /></button
													>
													<button
														type="button"
														class="btn btn-danger btn-icon waves-effect waves-light me-1"
														><i class="ri-google-fill fs-16" /></button
													>
													<button
														type="button"
														class="btn btn-dark btn-icon waves-effect waves-light me-1"
														><i class="ri-github-fill fs-16" /></button
													>
													<button
														type="button"
														class="btn btn-info btn-icon waves-effect waves-light"
														><i class="ri-twitter-fill fs-16" /></button
													>
												</div>
											</div>
										</form>
									</div>

									<div class="mt-5 text-center">
										<p class="mb-0">
											Already have an account ? <Link
												href="/authenticationInner/login/auth-signin-cover"
												class="fw-semibold text-primary text-decoration-underline"
											>
												Signin</Link
											>
										</p>
									</div>
								</div>
							</div>
						</Row>
					</Card>
				</Col>
			</Row>
		</Container>
	</div>

	<footer class="footer">
		<Container>
			<div class="row">
				<div class="col-lg-12">
					<div class="text-center">
						<p class="mb-0">
							&copy; {new Date().getFullYear()} Peer Club. Crafted with
							<i class="mdi mdi-heart text-danger" /> by Peerplays
						</p>
					</div>
				</div>
			</div>
		</Container>
	</footer>
</div>
