...
<div class="col">
	<Card>
		<Link href="" class={'card-header bg-soft-' + deal.bgColor} id={'leadDiscovered' + index}>
	...
	<Collapse toggler={'#leadDiscovered' + index} isOpen={true}>
		{#each deal.subItems as subitem, subitemkey}
			<Card class={subitem.isRibbon ? 'mb-1 ribbon-box ribbon-fill ribbon-sm' : 'mb-1'}>
				{#if subitem.isRibbon}
				...
				<CardBody>
					...
					>
						<div class="flex-shrink-0">
							<img src={subitem.img} alt="" class="avatar-xs rounded-circle" />
						</div>
						<div class="flex-grow-1 ms-3">
							<h6 class="fs-15 mb-1">{subitem.title}</h6>
							<p class="text-muted mb-0">
				...
				>
					<CardBody>
						<h6 class="fs-15 mb-1">
							{subitem.subTitle}{' '}
							<small class={'badge badge-soft-' + subitem.timeDurationClass}>
					...
					<CardFooter class="hstack gap-2">
						...
						<Button color="info" class="btn-sm w-100">
							<i class="ri-question-answer-line align-bottom me-1" />{' '}
							Message
						</Button>
					</CardFooter>
				</Collapse>
