	...
	const removeActivation = (items, target = null) => {
		...
		if (target && typeof target != undefined) {
			if (target.getAttribute('subitems')) {
				...
				target.nextElementSibling ? target.nextElementSibling.classList.add('show') : '';
			}
		}
		// }, 0);
	};
	...
	function changeClassAttribute(e, attribute, child = null, subchild = null, subsubchild = null) {
		...
		if (!e) {
			...
			setTimeout(() => {
				...
				document.querySelector("#navbar-nav a[id='" + current_child + "'].nav-link")
					? document
							.querySelector("#navbar-nav a[id='" + current_child + "'].nav-link")
							.classList.add('active')
					: '';
				// add active class to the submenu
				if (current_subchild)
					document.querySelector("#navbar-nav a[id='" + current_subchild + "'].nav-link")
						? document
								.querySelector("#navbar-nav a[id='" + current_subchild + "'].nav-link")
								.classList.add('active')
						: '';
							...
							{#if item.subItems}
								<li>
									<a
										href={null}
										subitems={item.id}
										class={current === item.id ? 'nav-icon active' : 'nav-icon'}
										id={item.stateVariables}
										on:click={(e) => changeClassAttribute(e, item.id)}
									>
										<i class={item.icon} />
									</a>
								</li>
											...
											<li class="nav-item">
												...
												>
													...
													{#if subItem.badgeName}
														<span
															class={'badge badge-pill bg-' + subItem.badgeColor}
															data-key="t-new"
														>
															{$_(subItem.badgeName)}
														</span>
										...
										{:else}
											<li class="nav-item">
												<Link
													...
													id={subItem.id}
													on:click={(e) => changeClassAttribute(e, item.id, subItem.id)}
												>
													{$_(subItem.label)}
												</Link>
												<div
													class={current_child === subItem.id
														? 'collapse menu-dropdown show'
														: 'collapse menu-dropdown'}
															...
															{#each subItem.childItems as childItem}
																{#if !childItem.childItems}
																	<li class="nav-item">
																		<a
																			...
																			class={current_subchild === childItem.id
																				? 'nav-link active'
																				: 'nav-link'}
																			href={childItem.link}
																			id={childItem.id}
																...
																{:else}
																	<span
																		...
																		id={childItem.id}
																		data-bs-toggle="collapse"
																		aria-expanded={current_subchild === childItem.id
																			? 'true'
																			: 'false'}
																		on:click={(e) =>
																			changeClassAttribute(e, item.id, subItem.id, childItem.id)}
																	>
																		{$_(childItem.label)}
																	</span>
																	<div
																		class="collapse menu-dropdown {current_subchild === childItem.id
																			? 'show'
																			: ''}"
																		id={item.id}
																	...
																	>
																		<ul class="nav nav-sm flex-column">
																			{#each childItem.childItems as subChildItem}
																				<li class="nav-item">
																					<a
																						class={current_sub_subchild === subChildItem.id
