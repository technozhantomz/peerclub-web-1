		...
		if (parentCollapseDiv) {
			...
			if (parent) {
				...
				if (parent.closest('.collapse.menu-dropdown')) {
					...
					if (parentParent) {
						parentParent
							.querySelector('span[data-bs-toggle]')
							.setAttribute('aria-expanded', 'true'); // 2nd parent dropdown
						if (parentParent.closest('.collapse.menu-dropdown')) {
							...
							var parentParentParent =
								parentParent.closest('.collapse.menu-dropdown').parentElement;
							if (parentParentParent) {
								parentParentParent
									.querySelector('span[data-bs-toggle]')
									.setAttribute('aria-expanded', 'true'); // 3nd parent dropdown
							}
						}
					}
				}
...
{#each data.Navdata as item}
	...
	{:else if item.subItems}
		<li class="nav-item">
			...
			<div class="collapse menu-dropdown {current === item.id ? 'show' : ''}">
							...
							<li class="nav-item">
								...
								>
									...
									{#if subItem.badgeName}
										<span class={'badge badge-pill bg-' + subItem.badgeColor}
											>{subItem.badgeName}</span
										>
									{/if}
								</a>
							</li>
										...
										{#if subItem.isChildItem}
											{#each subItem.childItems as childItem}
												{#if !childItem.childItems}
													<li class="nav-item">
														<a
															href={childItem.link ? childItem.link : null}
															on:click={() =>
