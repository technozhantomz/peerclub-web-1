		...
		if (parentCollapseDiv) {
			...
			if (parentCollapseDiv.parentElement.closest('.menu-dropdown')) {
				parentCollapseDiv.parentElement.closest('.collapse')
					...
					: '';
				var parentElementDiv = parentCollapseDiv.parentElement.closest('.collapse')
				...
				if (parentElementDiv) {
					...
					if (parentElementSibling) parentElementSibling.classList.add('active');
				}
	...
	{#if !item.isHeader}
		{#if item.subItems}
			<li class="nav-item">
				...
				<Collapse
					class={item.subItems.length > 13 ? 'menu-dropdown mega-dropdown-menu' : 'menu-dropdown'}
								...
								{#if index % 2 === 0}
									<Col lg={4}>
										<ul class="nav nav-sm flex-column">
											<li class="nav-item">
												<Link href={subItem.link} class="nav-link" on:click={callInitMenu}
